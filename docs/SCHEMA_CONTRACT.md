...
- **2026-09-03 — rushing charting unit: five `stats.rushing_*` raw tables
  ...
  - **`api.rushing_charting_direction_season` (new, `052`).** The tall
    ...
    consumer's to compute: `carries::numeric / NULLIF(direction_available_
    attempts, 0)` (or `/ NULLIF(direction_eligible_attempts, 0)`) -- NULL
    means no charted direction coverage for that entity/side, not an error;
    the fixed four-row melt guarantees rows with a zero denominator.

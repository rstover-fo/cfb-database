...
COMMENT ON VIEW api.passing_charting_team_season IS 'Team-season passing charting, offense and defense sides (2025+): season, team, conference, offense_total_air_yards, offense_average_depth_of_target, offense_air_yards_attempts_available, offense_total_yards_after_catch, offense_average_yards_after_catch, offense_yards_after_catch_attempts_available, and the defense_ equivalents. defense_* is this team''s passing DEFENSE (what opposing offenses did against them), not the opponent''s offensive row. NULL on a metric means those plays were not charted, 0 is a real observed value; the two *_attempts_available columns per side are the charting-coverage denominators. CONTRACT: each side''s yards_after_catch_attempts_available is bounded by that side''s completions (YAC exists only on completions; live-verified 152/152 rows 2026-08-31) -- completions are not exposed on this view (deliberately charting-family-only); request them if you need a per-catch YAC coverage ratio at team grain. Backed by marts.passing_charting_team_season.';

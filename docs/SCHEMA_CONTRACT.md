...
| `api.team_returning_production` | **Deployed** | 1,829 | Team-grain returning production rollup, 2021-2026. One row per team-season. Backed by `marts.team_returning_production`. Columns: team, season, returning_production_total, returning_production_offense, returning_production_defense, rp_qb, rp_rb, rp_wr_te, rp_ol, rp_dl, rp_lb, rp_db, rp_st, n_returning_starters, n_portal_in, n_portal_out, n_recruits_contributing, n_unknown_position, portal_trench_in_value, portal_trench_out_value, net_portal_trench_value, cfbd_returning_production_pct, our_pct_normalized, delta_vs_cfbd, generated_at. `net_portal_trench_value` (in − out, OL+DL only) surfaces who's winning the portal trench exchange — positive means net gain. CFBD calibration columns are NULL for 2026 until CFBD publishes `/player/returning`. |

...
1. **Two bases, two uses.** `ep_drive` = expected points of THIS possession
   ...
   recompute), render "not computed", never 0.
...
| `ep_net` | next-score basis (CFBD-comparable); negative when the opponent is favored to score next; NULL only after a partial recompute |
...
semantics are stable. `ep_net` populated 2026-08-08 (see the contract
changelog); UI copy can label the pair "house EP v1.5 (drive / next-score
basis)".

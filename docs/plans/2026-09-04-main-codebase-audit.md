...
**Implementation merged — September 5, 2026:** [PR #119](https://github.com/rstover-fo/cfb-database/pull/119) makes the three affected resources validate each response and propagate failures with endpoint/scope, original cause, and invocation-level request outcomes. The season summary and single/weekly/all-source CLI paths preserve failure status after earlier successes. [PR #120](https://github.com/rstover-fo/cfb-database/pull/120) adds the HTTP response status to structured failure summaries after a valid automated review finding. Regression coverage includes the failure matrix, valid empty responses, request limits, client cleanup, and dlt wrapping; independent review also exercised actual offline `pipeline.extract`. Full offline validation passed after the follow-up (2,260 root tests, 447 skipped; 59 MCP tests), and both PRs passed CI. See [operational failure reporting](../warehouse-operations.md#box-score-and-roster-request-failures). Live rollout, historical gap detection, and corrective backfills remain unverified; this update does not claim those are complete.
...
**Implementation prepared — September 5, 2026:** the builder now receives explicit regular/postseason targets from the season's `core.games` schedule. It emits each target from available plays strictly before that week, including unplayed future targets, and reuses a ridge solve when adjacent targets share the same inputs. The play-derived team layout, 150-play consumer threshold, prior-season/NULL fallback semantics, and per-season replacement remain intact. Offline regression coverage exercises numerical reference fits and an in-memory SQL fixture through staging replacement and actual feature/as-of lookup consumers. SQLite verifies the portable queries and fixture behavior; it does not verify Postgres server cursors, deployment state, or production outputs. See [weekly EPA operations](../warehouse-operations.md#weekly-epa-for-scheduled-games). Live rebuilds, historical feature/prediction repair, and the separate full-season live-blend path remain outside this change.

**Validation:** the original-main reproduction emitted no snapshots from week-one-only plays; the corrected builder emitted the explicit scheduled targets 2, 5, and 101. Full offline checks passed (2,264 root tests, 447 skipped; 59 MCP tests), along with Ruff lint/format and whitespace checks. Independent Astra review found no actionable defects and independently passed 58 weekly/integration/tuning-compatibility tests. No live warehouse writes or training runs were performed.

...
| `api.team_returning_production` | **Deployed** | 1,829 | Team-grain returning production rollup, 2021-2026. One row per team-season. Backed by `marts.team_returning_production`. Columns: team, season, returning_production_total, returning_production_offense, returning_production_defense, rp_qb, rp_rb, rp_wr_te, rp_ol, rp_dl, rp_lb, rp_db, rp_st, n_returners, n_portal_in, n_portal_out, n_recruits_contributing, n_unknown_position, portal_trench_in_value, portal_trench_out_value, net_portal_trench_value, cfbd_returning_production_pct, our_pct_normalized, delta_vs_cfbd, generated_at. `net_portal_trench_value` (in − out, OL+DL only) surfaces who's winning the portal trench exchange — positive means net gain. **Always-NOT-NULL**: team, season, returning_production_total, all rp_*, all n_*, all portal_trench_* columns (COALESCE'd to 0). **Nullable**: cfbd_returning_production_pct, our_pct_normalized, delta_vs_cfbd — all three are NULL for 2026 until CFBD publishes `/player/returning` (typically May/June). **Type contract**: returning_production_total numeric(8,3); rp_*/portal_trench_* numeric(7,3); our_pct_normalized/delta_vs_cfbd numeric(5,4); generated_at timestamptz. |
| `api.matchup_forecast` | **Deployed** | 45,897 | Game-level matchup forecast. One row per game in `core.games` with at least one team. Backed by `marts.pre_game_win_probability` LEFT JOINed to `marts.season_simulation_outcomes` (latest season only). Columns: game_id, season, week, season_type, start_date, completed, neutral_site, conference_game, home_team, away_team, market_spread, market_over_under, home_win_probability, away_win_probability, projected_winner, projected_margin, confidence_tier, cfbd_home_win_prob, market_home_win_prob, elo_home_win_prob, sp_home_win_prob, model_version, home_points, away_points, actual_winner, brier_loss, home_expected_wins, home_bowl_eligibility_prob, home_ten_plus_win_prob, away_expected_wins, away_bowl_eligibility_prob, away_ten_plus_win_prob. **Always-NOT-NULL**: game_id, season, week, home_team, away_team, home_win_probability, away_win_probability, model_version. **Nullable**: market_spread/market_over_under (no consensus line), home_points/away_points/actual_winner/brier_loss (game not yet completed), all home_*/away_* expected-wins columns (only populated for the latest season per the underlying matview's filter). **model_version**: opaque string (currently `v1_blended_cfbd_market_elo_sp`). A change to the value is breaking for probability semantics — clients should pin or version against it. |
...
| `marts.pre_game_win_probability` | Deployed | Game-level blended pregame win probability. One row per game in `core.games`. Combines CFBD pregame WP, market spread, Elo, and SP+ rating differentials with weights (0.50 / 0.25 / 0.15 / 0.10). Per-game brier_loss for completed games enables backtesting. ~45,897 rows. Exposed via `api.matchup_forecast`. |
| `marts.season_simulation_outcomes` | Deployed | Team-season outlook for the **latest season only**. One row per (season, team). 2,000-iteration Monte Carlo over per-game home_win_probability values. Columns: expected_wins, bowl_eligibility_prob, ten_plus_win_prob, win_distribution. ~699 rows. Joined into `api.matchup_forecast` for home/away outlook context. |
